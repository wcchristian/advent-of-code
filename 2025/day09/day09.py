...
def is_rect_inside_perimeter(p1, p2, perimeter):
    ...
    # Ensure no loop edge cuts through the rectangle interior.
    n = len(perimeter)
    for i in range(n):
        x_start, y_start = perimeter[i]
        x_end, y_end = perimeter[(i + 1) % n]

        if x_start == x_end:
            low_y, high_y = sorted((y_start, y_end))
            if min_x < x_start < max_x:
                if max(low_y, min_y) < min(high_y, max_y):
                    return False
        else:
            low_x, high_x = sorted((x_start, x_end))
            if min_y < y_start < max_y:
